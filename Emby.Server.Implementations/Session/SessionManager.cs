...
namespace Emby.Server.Implementations.Session
{
    ...
    public sealed class SessionManager : ISessionManager, IAsyncDisposable
    {
        ...
        /// <inheritdoc />
        public async Task SendVoiceChatSignal(string sessionId, MediaBrowser.Model.SyncPlay.VoiceChat.VoiceChatSignal signal, CancellationToken cancellationToken)
        {
            CheckDisposed();
            var session = GetSession(sessionId);
            await SendMessageToSession(session, SessionMessageType.VoiceChatSignal, signal, cancellationToken).ConfigureAwait(false);
        }

        /// <inheritdoc />
        public async Task SendVoiceChatState(string sessionId, MediaBrowser.Model.SyncPlay.VoiceChat.VoiceChatState state, CancellationToken cancellationToken)
        {
            CheckDisposed();
            var session = GetSession(sessionId);
            await SendMessageToSession(session, SessionMessageType.VoiceChatState, state, cancellationToken).ConfigureAwait(false);
        }
