...
public class SyncPlayController : BaseJellyfinApiController
{
    ...
    private readonly IVoiceChatManager _voiceChatManager;
    ...
    /// <param name="voiceChatManager">Instance of the <see cref="IVoiceChatManager"/> interface.</param>
    public SyncPlayController(
        ISessionManager sessionManager,
        ISyncPlayManager syncPlayManager,
        IUserManager userManager,
        IVoiceChatManager voiceChatManager)
    {
    ...
    /// <summary>
    /// Join voice chat in a SyncPlay group.
    /// </summary>
    /// <param name="groupId">The group identifier.</param>
    /// <response code="200">Voice chat joined successfully.</response>
    /// <response code="400">Voice chat is disabled or maximum participants reached.</response>
    /// <returns>The voice chat state.</returns>
    [HttpPost("VoiceChat/Join/{groupId:guid}")]
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    [Authorize(Policy = Policies.SyncPlayIsInGroup)]
    public async Task<ActionResult<MediaBrowser.Model.SyncPlay.VoiceChat.VoiceChatState>> JoinVoiceChat([FromRoute, Required] Guid groupId)
    {
        var currentSession = await RequestHelpers.GetSession(_sessionManager, _userManager, HttpContext).ConfigureAwait(false);

        try
        {
            var state = await _voiceChatManager.JoinVoiceChatAsync(
                groupId,
                currentSession.Id,
    ...
    /// <summary>
    /// Leave voice chat in a SyncPlay group.
    /// </summary>
    /// <param name="groupId">The group identifier.</param>
    /// <response code="204">Voice chat left successfully.</response>
    /// <returns>A <see cref="NoContentResult"/> indicating success.</returns>
    [HttpPost("VoiceChat/Leave/{groupId:guid}")]
    [ProducesResponseType(StatusCodes.Status204NoContent)]
    [Authorize(Policy = Policies.SyncPlayIsInGroup)]
    public async Task<ActionResult> LeaveVoiceChat([FromRoute, Required] Guid groupId)
    {
        var currentSession = await RequestHelpers.GetSession(_sessionManager, _userManager, HttpContext).ConfigureAwait(false);
        await _voiceChatManager.LeaveVoiceChatAsync(groupId, currentSession.Id, CancellationToken.None).ConfigureAwait(false);
        return NoContent();
    }
    ...
    /// <response code="204">Signal sent successfully.</response>
    /// <returns>A <see cref="NoContentResult"/> indicating success.</returns>
    [HttpPost("VoiceChat/Signal")]
    [ProducesResponseType(StatusCodes.Status204NoContent)]
    [Authorize(Policy = Policies.SyncPlayIsInGroup)]
    public async Task<ActionResult> SendVoiceChatSignal([FromBody, Required] MediaBrowser.Model.SyncPlay.VoiceChat.VoiceChatSignal signal)
    {
        var currentSession = await RequestHelpers.GetSession(_sessionManager, _userManager, HttpContext).ConfigureAwait(false);
        signal.FromSessionId = currentSession.Id;
        signal.Timestamp = DateTime.UtcNow;
        await _voiceChatManager.SendSignalAsync(signal, CancellationToken.None).ConfigureAwait(false);
        return NoContent();
    }

    /// <summary>
    /// Update mute status in voice chat.
    /// </summary>
    /// <param name="groupId">The group identifier.</param>
    /// <param name="isMuted">Whether the user is muted.</param>
    /// <response code="204">Mute status updated.</response>
    /// <returns>A <see cref="NoContentResult"/> indicating success.</returns>
    [HttpPost("VoiceChat/Mute/{groupId:guid}")]
    [ProducesResponseType(StatusCodes.Status204NoContent)]
    [Authorize(Policy = Policies.SyncPlayIsInGroup)]
    public async Task<ActionResult> UpdateVoiceChatMuteStatus(
        [FromRoute, Required] Guid groupId,
        [FromQuery, Required] bool isMuted)
    {
        var currentSession = await RequestHelpers.GetSession(_sessionManager, _userManager, HttpContext).ConfigureAwait(false);
        await _voiceChatManager.UpdateMuteStatusAsync(groupId, currentSession.Id, isMuted, CancellationToken.None).ConfigureAwait(false);
        return NoContent();
    }
    ...
    /// </summary>
    /// <param name="groupId">The group identifier.</param>
    /// <response code="200">Voice chat state retrieved.</response>
    /// <response code="404">Voice chat not active for this group.</response>
    /// <returns>The voice chat state.</returns>
    [HttpGet("VoiceChat/State/{groupId:guid}")]
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [Authorize(Policy = Policies.SyncPlayIsInGroup)]
    public ActionResult<MediaBrowser.Model.SyncPlay.VoiceChat.VoiceChatState> GetVoiceChatState([FromRoute, Required] Guid groupId)
    {
        var state = _voiceChatManager.GetVoiceChatState(groupId);
        if (state == null)
        {
            return NotFound("Voice chat not active for this group");
        }
    ...
    /// </summary>
    /// <response code="200">Voice chat configuration retrieved.</response>
    /// <returns>The voice chat configuration.</returns>
    [HttpGet("VoiceChat/Configuration")]
    [ProducesResponseType(StatusCodes.Status200OK)]
    public ActionResult<MediaBrowser.Model.SyncPlay.VoiceChat.VoiceChatConfiguration> GetVoiceChatConfiguration()
    {
        var config = _voiceChatManager.GetConfiguration();
        return Ok(config);
    }
}
